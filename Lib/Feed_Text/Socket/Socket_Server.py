...
def binder(client_socket, addr):
    # 커넥션이 되면 접속 주소가 나온다.
    log.info('Client Connecting.. --> Connected by' + str(addr));
    try:
        ...
        while True:
            ...
            if msg:
                # 수신된 메시지를 콘솔에 출력한다.
                log.info('Received from.. --> Socket Client ' + str(addr));
                log.info('Received Message from.. --> ' + str(msg));
            else:
...
server_socket.bind(('127.0.0.1', 8191));
